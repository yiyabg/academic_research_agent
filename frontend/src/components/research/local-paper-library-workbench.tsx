...
import type { LocalLibraryStatus, LocalPaper, LocalPaperAnalysisJob, LocalPaperSearchResponse, QueryInterpretation } from "@/types/literature-research";
...
const ADVANCED_FILTER_PLACEHOLDERS: Record<(typeof ADVANCED_FILTER_NAMES)[number], string> = {
  year_from: "起始年份",
  year_to: "截止年份",
  author: "作者",
  doi: "DOI",
...
export type LocalPaperAdvancedFilters = {
  year_from: string;
  year_to: string;
  author: string;
  doi: string;
...
export function buildLocalPaperSearchRequest(
  query: string,
  limit: number,
  filters: LocalPaperAdvancedFilters,
): Record<string, unknown> {
  const keywords = filters.keywords.split(/[,，]/).map((value) => value.trim()).filter(Boolean);
  return {
    query,
    limit,
    ...
    ...(filters.bibtex_type ? { bibtex_type: filters.bibtex_type } : {}),
    ...(keywords.length ? { keywords } : {}),
  };
}

export function queryInterpretationChips(interpreted: QueryInterpretation) {
  return [
    { key: "semantic", label: `语义：${interpreted.semantic_query || "（仅筛选）"}`, kind: "semantic" as const },
    ...Object.entries(interpreted.effective_filters).map(([key, value]) => ({
      key: `filter-${key}`,
      label: `${key}: ${Array.isArray(value) ? value.join("、") : String(value)}（${interpreted.filter_sources[key] ?? "—"}）`,
      kind: "filter" as const,
    })),
    ...interpreted.warnings.map((warning, index) => ({ key: `warning-${index}`, label: warning, kind: "warning" as const })),
  ];
}
...
function PaperRow({ paper }: { paper: LocalPaper }) {
  return <article className="rounded-lg border p-3 text-sm"><div className="flex items-start justify-between gap-2"><h4 className="font-medium">{paper.title}</h4><Badge variant="outline">{paper.source_kind.toUpperCase()}</Badge></div><p className="text-muted-foreground mt-1 text-xs">{paper.authors.join("; ")} · {paper.publication_year ?? "年份未知"}{paper.venue ? ` · ${paper.venue}` : ""}</p>{paper.keywords?.length ? <details className="mt-1 text-xs"><summary>关键词</summary>{paper.keywords.join("；")}</details> : null}{paper.evidence.map((e) => <blockquote key={`${e.page_number}-${e.chunk_index}`} className="mt-2 border-l-2 pl-2 text-xs">p.{e.page_number}: {e.text}</blockquote>)}</article>;
}
...
export function LocalPaperLibraryWorkbench() {
  ...
  const [query, setQuery] = useState(""); const [limit, setLimit] = useState(10); const [question, setQuestion] = useState(""); const [advanced, setAdvanced] = useState(false); const [filters, setFilters] = useState<LocalPaperAdvancedFilters>({ year_from: "", year_to: "", author: "", doi: "", venue: "", bibtex_type: "", keywords: "" });
  ...
  const search = useMutation({ mutationFn: (body: Record<string, unknown>) => literatureResearchApi.searchLocalLibrary(body), onSuccess: (data) => { setSearchResult(data); window.sessionStorage.setItem(SEARCH_RESULT_STORAGE_KEY, JSON.stringify(data)); }, onError: (e) => setMessage(getErrorMessage(e)) });
  ...
  const runSearch = (e: FormEvent) => { e.preventDefault(); setMessage(null); search.mutate(buildLocalPaperSearchRequest(query, limit, filters)); };
  ...
  return <Card className="border-primary/30"><CardHeader><CardTitle className="flex items-center gap-2"><Database className="size-5" />本地论文库（Zotero）</CardTitle><p className="text-muted-foreground text-sm">检索与问答已合并为可恢复、可审计的论文深度分析；仅使用本地页码证据。</p></CardHeader><CardContent className="space-y-4">
    ...
    <form onSubmit={runSearch} className="space-y-2"><div className="flex gap-2"><Input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="主题、关键词、作者、DOI、年份或 BibTeX 类型" /><Button disabled={search.isPending}><Search className="mr-1 size-4" />检索</Button></div><div className="flex items-center gap-2"><Label htmlFor="search-limit" className="text-xs">返回数量</Label><Input id="search-limit" type="number" min={1} max={50} value={limit} onChange={(e) => setLimit(Math.min(50, Math.max(1, Number(e.target.value))))} className="w-20 text-xs" /><Button type="button" variant="ghost" size="sm" onClick={() => setAdvanced((value) => !value)}>高级筛选</Button></div>{advanced && <div className="grid grid-cols-2 gap-2 rounded border p-2 md:grid-cols-4">{ADVANCED_FILTER_NAMES.map((name) => <Input key={name} value={filters[name]} type={name.startsWith("year") ? "number" : "text"} placeholder={ADVANCED_FILTER_PLACEHOLDERS[name]} onChange={(event) => setFilters((current) => ({ ...current, [name]: event.target.value }))} />)}</div>}</form>
    {searchResult && <div className="space-y-2"><p className="text-sm">命中 {searchResult.total} 篇，展示 {searchResult.items.length} 篇；检索追溯 ID：{searchResult.retrieval_run_id ?? "—"}</p>{searchResult.query_interpretation && <div className="flex flex-wrap gap-1 text-xs">{queryInterpretationChips(searchResult.query_interpretation).map((chip) => <Badge key={chip.key} variant={chip.kind === "warning" ? "destructive" : chip.kind === "filter" ? "outline" : "secondary"}>{chip.label}</Badge>)}</div>}{searchResult.items.map((paper) => <PaperRow key={paper.id} paper={paper} />)}<div className="flex flex-wrap gap-2">{(["markdown", "csv", "bibtex", "opml"] as const).map((f) => <Button key={f} size="sm" variant="outline" type="button" onClick={() => void download(f, query).catch((e) => setMessage(getErrorMessage(e)))}><FileText className="mr-1 size-3" />{f.toUpperCase()}</Button>)}</div></div>}
