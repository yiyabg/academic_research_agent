...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.UniqueConstraint(
            "paper_id", "source_sha256", "parser_version", "chunker_version",
            name="uq_local_paper_document_version",
        ),
    )
    op.create_index("ix_local_paper_document_versions_paper_id", "local_paper_document_versions", ["paper_id"])
    op.create_index("ix_local_paper_document_versions_status", "local_paper_document_versions", ["status"])
    op.create_index("ix_local_paper_document_versions_is_active", "local_paper_document_versions", ["is_active"])
    op.create_index(
    ...
    op.create_index("ix_local_papers_active_document_version_id", "local_papers", ["active_document_version_id"])
    ...
    op.add_column("local_paper_sections", sa.Column("heading_path_json", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default="[]"))
    op.add_column("local_paper_sections", sa.Column("section_type", sa.String(length=48), nullable=False, server_default="BODY"))
    op.add_column("local_paper_sections", sa.Column("page_end", sa.Integer(), nullable=True))
    op.add_column("local_paper_sections", sa.Column("token_count", sa.Integer(), nullable=False, server_default="0"))
    op.create_foreign_key("fk_local_paper_sections_document_version", "local_paper_sections", "local_paper_document_versions", ["document_version_id"], ["id"], ondelete="CASCADE")
    op.create_index("ix_local_paper_sections_document_version_id", "local_paper_sections", ["document_version_id"])
    op.create_index("ix_local_paper_sections_section_type", "local_paper_sections", ["section_type"])
    ...
    op.add_column("local_paper_chunks", sa.Column("token_count", sa.Integer(), nullable=False, server_default="0"))
    ...
    op.add_column("local_paper_chunks", sa.Column("lexical_tsv", postgresql.TSVECTOR(), nullable=True))
    op.create_foreign_key("fk_local_paper_chunks_document_version", "local_paper_chunks", "local_paper_document_versions", ["document_version_id"], ["id"], ondelete="CASCADE")
    op.create_index("ix_local_paper_chunks_document_version_id", "local_paper_chunks", ["document_version_id"])
    op.create_index("ix_local_paper_chunks_lexical_tsv", "local_paper_chunks", ["lexical_tsv"], postgresql_using="gin")
    ...
    op.create_table(
        ...
        sa.Column("id", uuid, primary_key=True),
        sa.Column("chunk_id", uuid, sa.ForeignKey("local_paper_chunks.id", ondelete="CASCADE"), nullable=False),
        ...
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=True),
        sa.UniqueConstraint("chunk_id", "page_number", "ordinal", name="uq_local_paper_chunk_locator"),
    )
    op.create_index("ix_local_paper_chunk_locators_chunk_id", "local_paper_chunk_locators", ["chunk_id"])

    op.create_table(
        "local_paper_tables",
        sa.Column("id", uuid, primary_key=True),
        sa.Column("paper_id", uuid, sa.ForeignKey("local_papers.id", ondelete="CASCADE"), nullable=False),
        sa.Column("document_version_id", uuid, sa.ForeignKey("local_paper_document_versions.id", ondelete="CASCADE"), nullable=False),
        sa.Column("table_index", sa.Integer(), nullable=False),
        sa.Column("page_number", sa.Integer(), nullable=False),
        sa.Column("bbox_json", postgresql.JSONB(astext_type=sa.Text()), nullable=True),
        sa.Column("caption_text", sa.Text(), nullable=True),
        sa.Column("structure_json", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default="{}"),
        sa.Column("html_text", sa.Text(), nullable=True),
        sa.Column("markdown_text", sa.Text(), nullable=True),
        sa.Column("created_at", sa.DateTime(timezone=True), nullable=False, server_default=sa.text("now()")),
        sa.Column("updated_at", sa.DateTime(timezone=True), nullable=True),
        sa.UniqueConstraint("document_version_id", "table_index", name="uq_local_paper_table_version"),
    )
    op.create_index("ix_local_paper_tables_paper_id", "local_paper_tables", ["paper_id"])
    op.create_index("ix_local_paper_tables_document_version_id", "local_paper_tables", ["document_version_id"])
    ...
    op.add_column("local_paper_figures", sa.Column("artifact_kind", sa.String(length=32), nullable=False, server_default="figure"))
    op.add_column("local_paper_figures", sa.Column("reference_texts_json", postgresql.JSONB(astext_type=sa.Text()), nullable=False, server_default="[]"))
    op.add_column("local_paper_figures", sa.Column("perceptual_hash", sa.String(length=128), nullable=True))
    op.create_foreign_key("fk_local_paper_figures_document_version", "local_paper_figures", "local_paper_document_versions", ["document_version_id"], ["id"], ondelete="CASCADE")
    op.create_foreign_key("fk_local_paper_figures_section", "local_paper_figures", "local_paper_sections", ["section_id"], ["id"], ondelete="SET NULL")
    op.create_index("ix_local_paper_figures_document_version_id", "local_paper_figures", ["document_version_id"])
    ...
    op.create_index("ix_local_paper_retrieval_runs_library_id", "local_paper_retrieval_runs", ["library_id"])
    op.create_index("ix_local_paper_retrieval_runs_owner_id", "local_paper_retrieval_runs", ["owner_id"])
...
def downgrade() -> None:
    ...
    op.drop_constraint("fk_local_paper_figures_document_version", "local_paper_figures", type_="foreignkey")
    ...
    op.drop_constraint("fk_local_paper_chunks_document_version", "local_paper_chunks", type_="foreignkey")
    ...
    op.create_unique_constraint(
        "uq_local_paper_section", "local_paper_sections", ["paper_id", "page_number", "section_index"]
    )
    ...
    op.drop_constraint("fk_local_paper_sections_document_version", "local_paper_sections", type_="foreignkey")
    ...
    op.drop_constraint("fk_local_papers_active_document_version", "local_papers", type_="foreignkey")
    ...
    op.drop_index("uq_local_paper_active_document_version", table_name="local_paper_document_versions")
