...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "paper_id", uuid, sa.ForeignKey("local_papers.id", ondelete="SET NULL"), nullable=True
        ),
    ...
    for field in ["job_id", "paper_id", "status", "provider_response_id"]:
        op.create_index(
            f"ix_local_paper_analysis_stages_{field}", "local_paper_analysis_stages", [field]
        )
    for field, foreign_table in [
        ("stage_id", "local_paper_analysis_stages"),
        ("paper_id", "local_papers"),
    ]:
        op.add_column(
            ...
            sa.Column(
                field,
                uuid,
                sa.ForeignKey(f"{foreign_table}.id", ondelete="SET NULL"),
                nullable=True,
            ),
        )
        op.create_index(
            f"ix_local_paper_analysis_llm_attempts_{field}",
            "local_paper_analysis_llm_attempts",
            [field],
        )
    op.add_column(
...
def downgrade() -> None:
    ...
    for field in ["paper_id", "stage_id"]:
        op.drop_index(
            f"ix_local_paper_analysis_llm_attempts_{field}",
            table_name="local_paper_analysis_llm_attempts",
        )
