...
def upgrade() -> None:
    ...
    op.create_table(
        ...
        sa.Column(
            "granted_by", uuid, sa.ForeignKey("users.id", ondelete="RESTRICT"), nullable=False
        ),
        sa.Column("permission", sa.String(length=24), nullable=False, server_default="ANALYZE"),
        *_timestamps(),
        sa.UniqueConstraint(
            "library_id", "project_id", name="uq_local_paper_library_project_grant"
        ),
    )
    op.create_index(
        "ix_local_paper_library_project_grants_library_id",
        "local_paper_library_project_grants",
        ["library_id"],
    )
    op.create_index(
        "ix_local_paper_library_project_grants_project_id",
        "local_paper_library_project_grants",
        ["project_id"],
    )
    op.create_table(
    ...
    for field in ["library_id", "owner_id", "project_id", "is_archived"]:
        op.create_index(
            f"ix_local_paper_analysis_sessions_{field}", "local_paper_analysis_sessions", [field]
        )
    op.create_table(
        ...
        sa.Column(
            "session_id",
            uuid,
            sa.ForeignKey("local_paper_analysis_sessions.id", ondelete="CASCADE"),
            nullable=False,
        ),
        sa.Column(
            "library_id",
        ...
        sa.Column(
            "retrieval_run_id",
            uuid,
            sa.ForeignKey("local_paper_retrieval_runs.id", ondelete="SET NULL"),
            nullable=True,
        ),
    ...
    for field in [
        ...
        op.create_index(
            f"ix_local_paper_analysis_jobs_{field}", "local_paper_analysis_jobs", [field]
        )
    op.create_index(
    ...
    op.create_table(
        ...
        sa.Column(
            "job_id",
            uuid,
            sa.ForeignKey("local_paper_analysis_jobs.id", ondelete="CASCADE"),
            nullable=False,
        ),
    ...
    op.create_index(
        "ix_local_paper_analysis_events_job_id", "local_paper_analysis_events", ["job_id"]
    )
    op.create_table(
    ...
    op.create_index(
        "ix_local_paper_analysis_turns_session_id", "local_paper_analysis_turns", ["session_id"]
    )
    op.create_index(
        "ix_local_paper_analysis_turns_job_id", "local_paper_analysis_turns", ["job_id"]
    )
    op.create_table(
    ...
    op.create_index(
        "ix_local_paper_analysis_llm_attempts_job_id",
        "local_paper_analysis_llm_attempts",
        ["job_id"],
    )
    op.create_table(
        ...
        sa.Column(
            ...
            uuid,
            sa.ForeignKey("user_research_profiles.id", ondelete="SET NULL"),
            nullable=True,
        ),
        *_timestamps(),
    )
    for field in ["owner_id", "session_id", "job_id", "status"]:
        op.create_index(
            f"ix_local_paper_memory_candidates_{field}", "local_paper_memory_candidates", [field]
        )
